...
function App() {
  ...
  const isPresent = (periodeAkhir, periodeBulanAkhir) => {
    if (periodeAkhir === new Date().getFullYear() && periodeBulanAkhir - 1 === new Date().getMonth()) {
      return false;
    } 
    if (new Date() > new Date(periodeAkhir, periodeBulanAkhir - 1)) {
      return true
    }
    return false;
  }
  ...
  return (
    <div className="App bg-container dark-mode overflow-hidden">
      ...
      <div onClick={()=> hamburger.setHamburgerActive(false)}>
                      ...
                      <>
                        <div className={`content py-5 px-10 h-fit md:w-[40%] flex flex-col gap-3 ml-[55px] md:ml-0 border-l-2 ${index % 2 ? "md:translate-x-[-50%] md:border-l-0 md:border-r-2" : "md:translate-x-[49.6%]"}`}>
                          ...
                          <div className={`${index % 2 ? "md:text-right" : null }`} data-aos={`${index % 2 ? "fade-right" : "fade-left" }`}>
                            ...
                            <h2 className='font-bold text-[12pt] text-oliveGreen'>{data.periodeAwal + " " + bulanNama[data.periodeBulanAwal - 1] + " - " + (isPresent(data.periodeAkhir, data.periodeBulanAkhir) ? data.periodeAkhir + " " + bulanNama[data.periodeBulanAkhir - 1] : 'present') }</h2>
                          </div>
